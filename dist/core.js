...
export async function searchJson(options) {
    const response = await postSearch(options, "application/json");
    return response.json();
}
export async function streamSearch(options, write) {
...
export function formatResponse(response, format, compact) {
    switch (format) {
        case "json": {
...
function buildCommand(state, env) {
...
function buildContents(state) {
    const contents = {};
    const shouldDefaultHighlights = state.highlightPreference === "auto" &&
...
function validateRequest(request) {
    ...
    if (typeof query !== "string" || query.trim() === "") {
        throw new CliError("A non-empty query is required. Pass positional query text, --query, or --body with query.");
    }
    ...
    if (request.category === "company" || request.category === "people") {
        const forbidden = ["excludeDomains", "startPublishedDate", "endPublishedDate"].filter((field) => request[field] !== undefined);
...
function validateContents(value) {
    if (!isRecord(value)) {
...
function writeSseContent(event, write) {
    const data = event
        .split(/\r?\n/)
...
function indentBlock(value, firstPrefix, nextPrefix) {
    return value
        .split(/\r?\n/)
...
function parseJsonOrFile(value, flag) {
...
function parseInteger(value, flag, bounds) {
    if (!/^-?\d+$/.test(value)) {
...
function assertIntegerValue(value, field, bounds) {
...
function validateStringArray(value, field, maxLength) {
    if (value === undefined) {
        return;
    }
