...
const packageName = "exa-search-cli";
...
const executablePackageFiles: string[] = ["bin/exa-search.js"];
...
async function buildPackageFromIndex(releaseVersion: string): Promise<string> {
  const temporaryRoot = await mkdtemp(join(tmpdir(), "exa-search-cli-release-"));
